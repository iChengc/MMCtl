...
class AddFriendAction : AccessibilityAction() {
    override fun execute(vararg args: Any?): ActionResult? {
        ...
        var result = Actions.execute(OpenWechatAction::class.java)
        if (!result!!.isSuccess()) {
            return result
        }
    ...
    private fun try2PerformAction(wechatId: String): ActionResult {
        var tryTimes = 0
    ...
    private fun performAction(wechatId: String): ActionResult {
        ...
        return if (!success) {
            ActionResult.failedResult("failed to back home page")
        } else ActionResult.successResult()
    }
